<template>
  <v-container>
    ...
    <h1 class="font-weight-bold mb-5 text-center">Pridėti naują veiklą</h1>
    ...
    <v-menu v-model="endTimeMenu" :close-on-content-click="false">
      ...
      ></v-time-picker>
        ...
        <v-btn color="primary" block large type="submit" class="mt-4">
          <v-icon left>mdi-check</v-icon> Pridėti veiklą
        </v-btn>
      </v-form>
    </v-card>
  </v-container>
 ...
 methods: {
  ...
  updateFormattedEndDate() {
   ...
   async submitEvent() {
    ...
    const startDateObject = new Date(this.event.startDate);
    const endDateObject = new Date(this.event.endDate);
   ...
   day: "2-digit"
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.message || "Klaida kuriant veiklą");
      }

      this.toast.success("Veikla sėkmingai sukurta!");
      this.$router.push("/events");
